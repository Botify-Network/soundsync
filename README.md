# SoundCloud Auto Sync/Downloader

Windows Electron tray app that auto-syncs SoundCloud likes and playlists to a local folder using **yt-dlp** + **ffmpeg**.
...
- Current release: [v2.0.5](https://github.com/HIGH-TEXAS-LIST/sc-auto-downloader/releases/tag/v2.0.5)
...
- `SoundCloud Auto Sync Setup <version>.exe` (the NSIS installer produced by `electron-builder`)
...
3. Upload both to the release:
   `gh release upload v<version> "dist/SoundCloud Auto Sync Setup <version>.exe" dist/latest.yml`
...
Added in v2.0.5 (Settings → Startup / App Updates card):
...
Update errors now surface as tray balloons (previously console-only).

## Security / hardening highlights (v2.0.5)

- **Renderer XSS fix** — settings UI no longer injects user-supplied usernames / playlist URLs / titles via `innerHTML`. All list-item construction uses `createElement` + `textContent`.
- **Safer process execution** — main-process diagnostics and `fetch-playlist-metadata` (which receives a user-controlled URL) no longer shell-interpolate. All `yt-dlp` and `ffmpeg` invocations use argv arrays via `spawn` / `execFile`.
- **CLI hardening** — `cli.js` shell-interpolated calls converted to `execFileSync` with argv arrays.
- **Rate-limit / network hardening** — every `yt-dlp` call now passes:
  ...
  - Per-track 1.5 s pacing in `getFullMetadata` (was: back-to-back; the single biggest 429 vector)
...
```bash
npm run bump            # patch:  2.0.5 → 2.0.6
npm run bump:minor      # minor:  2.0.5 → 2.1.0
npm run bump:major      # major:  2.0.5 → 3.0.0
```
...
Note: the CLI maintains its own config at `%APPDATA%\soundcloud-auto-sync\config.json` (separate from the Electron app's `electron-store`), and its `sync` path does not yet share the rate-limit hardening that the Electron sync uses.
